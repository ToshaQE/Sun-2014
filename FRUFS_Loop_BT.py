...
sun_y_train_destat = Model_Data.destat_data["y_train"]
...
for n in list(range(1, total_n_features+1)):
    ...
    y_pred_in = frufs_model.predict()
    MAE_train = np.nanmean(abs(y_pred_in - sun_y_train.squeeze()))
    ...
    # De-stationarising the data anc calculating the destated MAE
    
    if y_integ_order > 0:
        if stationarity_method == 0:
            y_pred_in_destat = y_pred_in.copy()
            y_pred_in_destat.loc[-1] = sun_y_train_destat.iloc[0]
