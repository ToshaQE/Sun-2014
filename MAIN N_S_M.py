...
def algo(df, target, max_lag, stationarity_method, test_size):
    ...
    y_original = y_original.iloc[1:]
    y_original.reset_index(drop=True, inplace=True)
    ...
    for i in list(range(max_lag)):
        model = AutoReg(y_train, lags=i, trend="n").fit()
        BICs.append(model.aic)
    ...
    try:
        ...
        feature_n_dfs_merge_copy = feature_n_dfs_merge
        ...
        columns_y_o = []
        for i in list(range(1, min_bic_ind+1)):
            columns_y_o.append(target+".L"+str(i))

        y_o_lags_df = pd.DataFrame(columns=columns_y_o)
        for i in list(range(min_bic_ind)):
            y_o_lags_df[columns_y_o[i]] = y_original.shift(i+1)

        # Truncating lags of y at the maximum lag length
        y_o_lags_df = y_o_lags_df.iloc[max_lag:,:]

        # bic_selected_l_of_y = list(y_lags_df.columns)
        # y_o_lags_df = y_o_lags_df[bic_selected_l_of_y]

        y_o_lags_df.reset_index(drop=True, inplace=True)

        feature_n_dfs_merge_copy[0] = y_o_lags_df
        feature_n_dfs_merge_copy = pd.concat(feature_n_dfs_merge_copy, axis=1)

        y_original = y_original.iloc[max_lag:]
        y_original.reset_index(drop=True, inplace=True)



        fin_model = AutoReg(y_original, lags=0, exog=feature_n_dfs_merge_copy).fit()
        ...
        while max_p_value >= critical_p_value:
            ...
            if least_sig_var == "const":
                fin_model = AutoReg(y_original, lags=0, exog=feature_n_dfs_merge_copy, trend="n").fit()
                const_dropped = True

            else:
                # Dropping the least_sig_var from the df
                feature_n_dfs_merge_copy.pop(least_sig_var)
        ...
        MAE_train = np.nanmean(abs(y_pred_in - y_original))
        MSE_train = mean_squared_error(y_pred_in, y_original)
        RMSE_train = math.sqrt(MSE_train)
        RAE_train = rae(actual=y_original, predicted = y_pred_in)
        RRSE_train = rrse(actual=y_original, predicted = y_pred_in)
        ...
        # Truncating y_test, so its length corresponds to that of y_lags_df
        ...
        Model_Data = Sun_Model(fin_model, fin_model.summary(), aug_models, MAE_,
                                y_original, feature_n_dfs_merge_copy,
                                y_test, test_data,
...
aapl_medium = df_aapl.iloc[:2500,:22]
