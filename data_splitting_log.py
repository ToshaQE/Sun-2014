...
def algo(df, target, max_lag, test_size):
    ...
    for feature in features:
        feature_logged = np.log(df[featudre])
        inf_sum = np.isinf(feature_logged).sum()
        if inf_sum >= 1:
            # adding a constant in case the original series contains zeroes
            feature_logged_with_c = np.log(df[feature] + 1)
            df[feature] = feature_logged_with_c
        result = adfuller(df[feature], autolag="t-stat")
        counter = 0
        while result[1] > 0.01:
            feature_differenced = df[feature] - df[feature].shift(1)
            ...
            if na_count > 1:
                ...
                feature_differenced = feature_with_constant - \
                    feature_with_constant.shift(1)
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        feature_df, target_df, test_size=test_size, shuffle=False)
    ...
    for n in list(range(len(Xs))):
        ...
        for i in list(range(max_lag-1)):
            model = AutoReg(
                y_train_m, lags=0, exog=y_and_x_lags_df.iloc[:, :i+len(list(y_lags_df.columns))+1]).fit()
            BICs.append(model.bic)

        min_bic_ind_aug = BICs.index(min(BICs))
        # Full and Partial autocorrelation plot?
        feature_n_df1 = y_and_x_lags_df
        y_and_x_lags_df = y_and_x_lags_df.iloc[:,
                                               :min_bic_ind_aug+len(list(y_lags_df.columns))+1]
        y_and_x_lags_df.reset_index(drop=True, inplace=True)

        model = AutoReg(y_train_m, lags=0, exog=y_and_x_lags_df).fit()

        gr_test_df = pd.concat([X_train[Xs[n]], y_train], axis=1)
        granger_p_stat = grangercausalitytests(
            gr_test_df, maxlag=[min_bic_ind_aug+1])[min_bic_ind_aug+1][0]['params_ftest'][1]
        if granger_p_stat >= 0.05:
            aug_models[Xs[n]] = model
            feature_n_dfs[Xs[n]] = feature_n_df1
            feature_n_dfs_merge.append(
                y_and_x_lags_df.iloc[:, len(list(y_lags_df.columns)):])
    ...
    try:
        ...
        y_pred_out = fin_model.predict(
            start=first_oos_ind, end=last_oos_ind, exog_oos=test_data)
        ...
        logging.error(
            "Can not reject that the target variable 'reverse causes' independent features.")
