...
def algo(df, target, max_lag, stationarity_method, test_size):
    ...
    y_original_only = y_original.copy()
    ...
    try:
        ...
        if orders_of_integ[target] > 0:
            ...
            if stationarity_method == 0:
                ...
                y_train_m_destat.loc[-1] = y_original_only.iloc[max_lag]
                ...
                y_pred_in_destat = y_train_m_destat.iloc[:-1] + y_pred_in

                y_train_m_destat = y_train_m_destat.iloc[1:]
                y_train_m_destat.reset_index(drop=True, inplace=True)
                MAE_train_destat = np.nanmean(abs(y_pred_in_destat - y_train_m_destat))
                # y_original_only = y_original_only.iloc[max_lag:]
                # y_original_only.reset_index(drop=True, inplace=True)
                # y_pred_in_destat = y_original_only.iloc[:-1] + y_pred_in
                
                # y_original_only = y_original_only.iloc[1:]
                # y_original_only.reset_index(drop=True, inplace=True)
                # MAE_train_destat = np.nanmean(abs(y_pred_in_destat - y_original_only.iloc[1:]))
